function customerManage(customerManageConfig){
	
	//--------------initialize------------------------//
	var _customerManageConfig = customerManageConfig;
	var table = new Table($("#customerList"),_customerManageConfig.tableConfig);
	...
	$("form button").bind("click",function(){
		location.href = _customerManageConfig.URL.DETAILS;//"customerOwner/customerDetail.html";
	});
	...
	function bindBlacklistEvent(){//推迟绑定事件到加载完成之后 ajax获取到数据并且完成table渲染时绑定
		$(".blacklist_btn").bind("click",function(){
			...
			baseAjax.doAjax(_customerManageConfig.URL.BLACKLIST.replace("{userId}", userId), null, function(rs){
				if(_customerList.length==1 && curPageIdx>1){//如果当前页面只剩下一项，删掉后就必须跳回前一个页面
					curPageIdx--;
				}
				loadData();
			},function(rs){
	...
	function bindNormalEvent(){//推迟绑定事件到加载完成之后 ajax获取到数据并且完成table渲染时绑定
		$(".normal_btn").bind("click",function(){
			var userId = $(this).attr("userId");
			baseAjax.doAjax(_customerManageConfig.URL.NORMAL.replace("{userId}", userId), null, function(rs){
				if(_customerList.length==1 && curPageIdx>1){//如果当前页面只剩下一项，删掉后就必须跳回前一个页面
					curPageIdx--;
				}
				loadData();
	...
	function loadData(){
		...
		baseAjax.doAjax(_customerManageConfig.URL.LIST, queryParam, function(rs){
			/*data 的格式如下  {
			 *  code : 0 , 
			 *  result : [{
			 *  	storeId : '',
			 ...
			 */
			var page = rs.result;
			_customerList = page.data;
			table.loadData(_customerList);
